...
class UFOSpawner:
    ...
    def spawn_random_ufo(self):
        ...
        if edge == "left":
            position = pygame.Vector2(-UFO_RADIUS, random.uniform(0, SCREEN_HEIGHT))

        elif edge == "right":
            position = pygame.Vector2(
                SCREEN_WIDTH + UFO_RADIUS, random.uniform(0, SCREEN_HEIGHT)
            )

        elif edge == "top":
            position = pygame.Vector2(random.uniform(0, SCREEN_WIDTH), -UFO_RADIUS)

        elif edge == "bottom":
            position = pygame.Vector2(
                random.uniform(0, SCREEN_WIDTH), SCREEN_HEIGHT + UFO_RADIUS
            )
