...
class DebugManager:
    ...
    def draw(self, screen, fps, counts):
        ...
        debug_header_rect = debug_header.get_rect(topright=(screen.get_width() - 20, 20))
        screen.blit(debug_header, debug_header_rect)

        debug_fps = self.text_font.render(f"FPS: {fps:.0f}", True, "white")
        debug_fps_rect = debug_fps.get_rect(topright=(screen.get_width() - 20, debug_header_rect.bottom + 5))
        screen.blit(debug_fps, debug_fps_rect)

        next_y = debug_fps_rect.bottom + 5

        for label, value in counts.items():
            debug_line = self.text_font.render(f"{label}: {value}", True, "white")
            debug_line_rect = debug_line.get_rect(topright=(screen.get_width() - 20, next_y))
        ...
        for control in controls:
            control_line = self.controls_font.render(control, True, "white")
            control_line_rect = control_line.get_rect(topright=(screen.get_width() - 20, next_y))
