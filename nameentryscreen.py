...
class NameEntryScreen:
    ...
    def handle_event(self, event, input_manager):
        ...
        controller_character_pressed = input_manager.is_name_character_pressed(event)
        ...
        if controller_character_pressed and len(self.name) < self.max_length:
            selected_character = NAME_CHARACTERS[self.selected_character_index]
        ...
        if input_manager.has_controller():
            ...
            for slot_index in range(self.max_length):
                ...
                if slot_index < len(self.name):
                    ...
                    displayed_character = NAME_CHARACTERS[self.selected_character_index]
                ...
                character_rect = character_text.get_rect(center=slot_rect.center)
            ...
            if current_slot < self.max_length:
                selector_x = starting_x + current_slot * slot_width + slot_size // 2

                previous_index = (self.selected_character_index - 1) % len(
                    NAME_CHARACTERS
                )

                next_index = (self.selected_character_index + 1) % len(NAME_CHARACTERS)
